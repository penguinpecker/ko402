...
export interface GameRoom {
  ...
  txLog: MoveTx[]; lastUpdate: number; startedAt: number;
}
...
function rowToRoom(row: any): GameRoom {
  return {
    ...
    txLog: row.tx_log || [], lastUpdate: new Date(row.updated_at).getTime(), startedAt: row.started_at ? new Date(row.started_at).getTime() : 0,
  };
}
...
export async function getRoom(): Promise<GameRoom | null> {
  ...
  if (!data) return null;
  // NO auto-expire — only manual reset via DELETE /api/game/lobby
  return rowToRoom(data);
}

export async function resetRoom(): Promise<void> {
  await supabase.from('ko402_rooms').upsert({
    id: ROOM_ID, status: 'empty', p1: null, p2: null, turn: 0, current_turn: 'p1',
...
export async function joinRoom(wallet: string, fighter: string, name: string): Promise<{ slot: 'p1' | 'p2'; room: GameRoom } | { error: string }> {
  ...
  if (!data) {
    await resetRoom();
    const { data: fresh } = await supabase.from('ko402_rooms').select('*').eq('id', ROOM_ID).single();
    if (!fresh) return { error: 'Failed to create room' };
  }
  
  // Re-read
  const { data: room } = await supabase.from('ko402_rooms').select('*').eq('id', ROOM_ID).single();
  if (!room) return { error: 'No room' };

  // If previous game ended, reset for new game
  if (room.status === 'ko') {
    await resetRoom();
    const { data: fresh } = await supabase.from('ko402_rooms').select('*').eq('id', ROOM_ID).single();
    if (!fresh) return { error: 'Reset failed' };
    // Re-enter with fresh room
    return joinRoom(wallet, fighter, name);
  }

  // Already joined?
  if (room.p1?.wallet === wallet) return { slot: 'p1', room: rowToRoom(room) };
  if (room.p2?.wallet === wallet) return { slot: 'p2', room: rowToRoom(room) };

  if (room.status === 'fighting') return { error: 'Game in progress. Reset with DELETE /api/game/lobby' };
  ...
  if (!room.p2) {
    const { data: u } = await supabase.from('ko402_rooms')
      .update({ p2: agent, status: 'fighting', pot: 0.2, current_turn: 'p1', started_at: new Date().toISOString(), updated_at: new Date().toISOString() })
      .eq('id', ROOM_ID).select().single();
...
export async function submitMove(
  ...
  if (!data || data.status !== 'fighting') {
    return { success: false, room: data ? rowToRoom(data) : {} as GameRoom, error: 'No active game (status: ' + data?.status + ')' };
  }
  ...
  if (data.current_turn !== expectedTurn) {
    return { success: false, room: rowToRoom(data), error: 'Not your turn' };
  }
  ...
  const updates: any = {
    turn: (data.turn || 0) + 1, tx_log: txLog, current_turn: nextTurn,
    updated_at: new Date().toISOString(),
    ...(isP1 ? { p1: attacker, p2: defender } : { p2: attacker, p1: defender }),
  };

  if (defender.hp <= 0) { updates.status = 'ko'; updates.winner = attacker.name; }
...
export async function checkTimeout(): Promise<GameRoom | null> {
  const { data } = await supabase.from('ko402_rooms').select('*').eq('id', ROOM_ID).single();
  if (!data || data.status !== 'fighting' || !data.started_at) return data ? rowToRoom(data) : null;

  const elapsed = (Date.now() - new Date(data.started_at).getTime()) / 1000;
  if (elapsed >= 60) {
    const winner = (data.p1?.hp || 0) >= (data.p2?.hp || 0) ? data.p1?.name : data.p2?.name;
    const { data: updated } = await supabase.from('ko402_rooms')
      .update({ status: 'ko', winner, updated_at: new Date().toISOString() })
      .eq('id', ROOM_ID).select().single();
    return updated ? rowToRoom(updated) : null;
  }
  return rowToRoom(data);
}
